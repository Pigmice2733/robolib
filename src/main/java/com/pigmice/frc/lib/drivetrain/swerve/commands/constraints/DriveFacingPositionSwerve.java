...
public class DriveFacingPositionSwerve extends CommandBase {
    ...
    // /**
    // * Field oriented DriveWithJoysticks with the robot always facing the
    // specified
    // * target positions
    // */
    // public DriveFacingPositionSwerve(SwerveDrivetrain drivetrain,
    // Supplier<Double> driveSpeedX,
    ...
    // double angle = Math
    // .toDegrees(Math.atan2(targetPos.getY() - robotPose.getY(), targetPos.getX() -
    // robotPose.getX()));
    // double radPerSec =
    // controller.calculate(drivetrain.getPose().getRotation().getDegrees(), angle);

    // drivetrain.driveChassisSpeeds(ChassisSpeeds.fromFieldRelativeSpeeds(driveSpeedY.get(),
    // driveSpeedX.get(),
