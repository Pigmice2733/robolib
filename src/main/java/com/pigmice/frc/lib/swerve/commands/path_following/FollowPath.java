...
public class FollowPath extends SequentialCommandGroup {
     ...
     */
    public FollowPath(SwerveDrivetrain drivetrain, PathPlannerTrajectory trajectory, HashMap<String, Command> eventMap) {
        SwerveConfig config = drivetrain.config;
        addCommands(
            drivetrain.resetOdometryCommand(trajectory.getInitialHolonomicPose()),
     ...
     * @param reversed reverse the robots direction
     */
    public FollowPath(SwerveDrivetrain drivetrain, String pathName, HashMap<String, Command> eventMap, boolean reversed) {
        this(
                drivetrain,
                PathPlanner.loadPath(
                        pathName,
